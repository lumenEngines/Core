...
from utils.file_tracker import file_tracker
...
class ProjectSummarizationThread(QThread):
    ...
    def run(self):
        try:
            ...
            file_tracker.track_project_files(self.file_paths)
...
class ProjectManagerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Only start file monitoring when user explicitly selects a project
        # self.file_watcher_timer.start(5000)  # Will start when project is selected
    ...
    def setup_details_panel(self, parent):
        ...
        # Create vertical splitter for content and summary
        ...
        # Set vertical splitter proportions - more space for content
        content_splitter.setStretchFactor(0, 3)  # File content gets more space
        content_splitter.setStretchFactor(1, 2)  # Summary gets less space
    ...
    def on_project_changed(self, project_name):
        ...
        if success:
            ...
            self.load_project_files()
            self.file_watcher_timer.start(5000)  # Start monitoring
            print(f"Selected project: {project_name}")
            
            # Build fast lookup for instant code detection
            print("Building fast lookup for instant code detection...")
            instant_detector.build_fast_lookup(project_name)
        else:
    ...
    def check_file_changes(self):
        ...
        try:
            changes = file_tracker.check_for_changes()
            ...
            for change in changes:
                ...
                if change_type == 'modified':
                    # Re-summarize modified file
                    self.update_file_tree_status(file_path, "Updating...")
                    summary_data = project_summarizer.file_summarizer.summarize_file(file_path)
                    
                    if summary_data:
                        self.file_summaries[file_path] = summary_data
                        self.update_file_tree_status(file_path, "Summarized")
                        
                        # Update display if this file is currently selected
                        current_item = self.file_tree.currentItem()
                        if current_item and current_item.data(0, Qt.UserRole) == file_path:
                            self.display_file_details(file_path)
                    else:
                    ...
                    self.update_file_tree_status(file_path, "Deleted")
